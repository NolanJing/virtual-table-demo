...
const MOCK_LIST_LENGTH = 100;
const mockItemTempalte = {
...
const list = [];
function getMockResourceQuoteList(){
  return Mock.mock({"resource_quote_list|1-10": [
    {
      "tech_solution_resource_detail_id|+1": 1,
      "resource_alias": Random.string( 5, 10),
      "product": {
...
for (let index = 0; index < MOCK_LIST_LENGTH; index++) {
  const item = Mock.mock(mockItemTempalte);
  item.resource_quote_list =  getMockResourceQuoteList()
  item.site.name = Random.cword(5, 7)
  list.push(item)
